...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1>Store Page</h1>
        @can('insert-product')
        <a href="{{ route('product_insert_form') }}" class="btn btn-primary">Insert Product</a>
        @endcan
    </div>
        ...
        @foreach ($products as $p)
            <div class="col">
                <div class="card h-100 shadow-sm">
                    <img src="{{ $p->image_path ? asset('product_images/' . $p->image_path) : 'https://placehold.co/200x200?text=No+Image' }}" 
                         class="card-img-top" 
                         alt="{{ $p->name }}" 
                    ...
                    <div class="card-body">
                        ...
                        <p class="card-text text-muted"><i>{{ $p->details }}</i></p>
                        <p class="card-text fw-bold">Rp {{ number_format($p->price, 0, ',', '.') }}</p>
                    </div>
                    <!-- Add to Cart Trigger -->
                  ...
                  <div class="modal fade" id="addToCartModal{{ $p->id }}" tabindex="-1" aria-labelledby="addToCartModalLabel{{ $p->id }}" aria-hidden="true">
                    ...
                    <div class="card-footer bg-transparent border-top-0 pb-3">
                        @can('edit-product')
                            <a href="{{ route('product_edit_form', $p->id) }}" class="btn btn-warning">Edit</a>
                        @endcan
